...
mapParser f (P p) = P $ \input -> case p input of
...
bindParser f (P p) = P $ \input -> case p input of
                                       ErrorResult e -> ErrorResult e
...
list1 p = flbindParser p (\p' -> flbindParser (list p)
                             (\p'' -> valueParser (p' :. p'')))
...
-- True
natural :: Parser Int
natural = bindParser (\x -> case read x of
                                Empty  -> failed
...
sequenceParser (x:.xs) = flbindParser x (\a -> flbindParser (sequenceParser xs)
                                            (\as -> valueParser (a :. as)))
...
firstNameParser = flbindParser upper (\c -> flbindParser (list lower)
                                         (\cs -> valueParser (c :. cs)))
...
surnameParser = flbindParser upper (\c -> flbindParser (thisMany 5 lower)
                                       (\cs -> flbindParser (list lower)
                                           (\a -> valueParser (c :. cs ++ a))))
...
phoneParser = flbindParser digit (\d -> flbindParser phoneBodyParser
                                     (\a -> flbindParser (is '#')
                                         (\_ -> valueParser (d :. a))))
...
personParser = flbindParser ageParser
               (\a -> spaces1 >>> flbindParser firstNameParser
                     (\f -> spaces1 >>> flbindParser surnameParser
                            (\s -> spaces1 >>> flbindParser smokerParser
                                   (\g -> spaces1 >>> flbindParser phoneParser
...
instance Applicative Parser where
    ...
    p1 <*> p2 = bindParser (\f -> bindParser (valueParser . f) p2) p1
