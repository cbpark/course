...
instance Monad f => Applicative (StateT s f) where
    ...
    StateT f <*> StateT k = StateT $ (\(g, s) -> first g <$> k s) <=< f
...
instance Monad f => Monad (StateT s f) where
    ...
    f =<< StateT k = StateT $ (=<<) (\(a, s) -> runStateT (f a) s) . k
...
distinct' x = eval' (filtering (\a -> state' (S.notMember a &&& S.insert a)) x)
              S.empty
...
distinctG x =
    ...
    evalT (filtering
           (\a -> StateT
               (\s -> OptionalT
                      (if a > 100
                       then log1 (fromString ("> 100: " P.++ show a)) Empty
                       else (if even a
                             then log1 (fromString ("even: " P.++ show a))
                             else pure)
                            (Full (S.notMember a s, S.insert a s))))) x) S.empty
