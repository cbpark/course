...
readFile = fmap listh . P.readFile . hlist

writeFile :: Filename -> Chars -> IO ()
writeFile n s = P.writeFile (hlist n) (hlist s)

getLine :: IO Chars
getLine = fmap listh P.getLine

getArgs :: IO (List Chars)
getArgs = fmap (listh . fmap listh) E.getArgs
        ...
        perms (t:.ts) is =
            let interleave' _ Nil     r = (ts, r)
                interleave' f (y:.ys) r =
                    ...
                    in  (y:.us, f (t:.y:.us):.zs)
            in foldRight (\xs -> snd . interleave' id xs) (perms ts (t:.is))
               (permutations is)
...
reads :: Read a => Chars -> Optional (a, Chars)
reads s = case P.reads (hlist s) of
              []         -> Empty
              ((a, q):_) -> Full (a, listh q)

read :: Read a => Chars -> Optional a
